...
export function exportRawData(sessionId: string): object {
  // 从 events 表查询所有事件 → 按回合/阶段分组 → 输出 raw-data.json
}

export function exportLabeledData(sessionId: string): object {
  // 遍历所有矿工的所有轮次的声明/真实/审计结果 → 输出 labeled-data.json
}
```
